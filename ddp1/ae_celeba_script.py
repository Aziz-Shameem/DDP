...
def train_epoch(model, loader, p1, p2) :
    ...
    for images in tqdm(loader):
        ...
        recon = model(images)
        if torch.randn(1)<=p1 : recon = model(add_sp_noise(images.clone())) # denoising
        elif torch.randn(1)<=p2 : recon = model(mask(images.clone())) # masking
        else : recon = model(images)
        loss = model.compute_loss(images, recon)
...
if __name__ == '__main__' :
    ...
    for epoch in tqdm(range(args.epochs), disable = args.silent): 
        ...
        if epoch%10==0 : torch.save(model, args.savedir+f'/model{epoch}.pt')
